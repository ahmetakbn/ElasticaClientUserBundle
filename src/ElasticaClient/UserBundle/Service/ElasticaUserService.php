...
class ElasticaUserService implements UserServiceInterface {
	...
	public function getByEmail($userEmail) {
		...
		$query1 = new Query( array('query' => array('bool' => array('must' => array('match' => array('email' => $emailParts[0]))))));
		$result1 = $this -> index -> search($query1) -> getResults();

		$query2 = new Query( array('query' => array('bool' => array('must' => array('match' => array('email' => $emailParts[1]))))));
